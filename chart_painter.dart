...
class ChartPainter extends BaseChartPainter {
  ...
  // 新增用于动画过渡的字段
  List<KLineEntity>? _prevDatas;
  double _animationOpacity = 1.0;
  bool _dataChanged = false; // 数据更新标记
  ...
  void initChartRenderer() {
    ...
    // 动画处理逻辑：如果数据有更新，则将透明度重置为0以实现渐变过渡
    if (_prevDatas != null && datas != null && datas!.isNotEmpty) {
      // 判断数据是否有更新
      if (_prevDatas!.isNotEmpty && _prevDatas!.last.time != datas!.last.time) {
        _dataChanged = true;
        _animationOpacity = 0.0;
      } else {
        _dataChanged = false;
      }
    }

    _prevDatas = datas != null ? List.from(datas!) : null;
  }
  ...
  void drawChart(Canvas canvas, Size size) {
    ...
    // 动画过渡：在绘制前先开启一层Layer
    // 当数据更新时，_animationOpacity从0逐渐过渡到1，使图表从浅到深显示
    ...
    // 如果数据刚有更新，则让_opacity从0慢慢递增到1
    if (_dataChanged && _animationOpacity < 1.0) {
      _animationOpacity += 0.05; // 每次刷新稍稍提高透明度，值可自行调整
      if (_animationOpacity > 1.0) {
        _animationOpacity = 1.0;
      }
    }

    // 使用一个浅色半透明图层叠加在上面，实现从浅到深的过渡
    Paint fadePaint = Paint()..color = Colors.white.withOpacity(1.0 - _animationOpacity);
    canvas.drawRect(Rect.fromLTWH(-mTranslateX, 0, size.width/scaleX, size.height), fadePaint);
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    TextPainter tp = getTextPainter(formatWithComma(point.close.toString(), -1), chartColors.crossTextColor);
  ...
  void drawMaxAndMin(Canvas canvas) {
    ...
    if (x < mWidth / 2) {
      TextPainter tp = getTextPainter(
        formatWithComma(mMainLowMinValue.toStringAsFixed(fixedLength), -1),
    ...
    if (x < mWidth / 2) {
      TextPainter tp = getTextPainter(
        formatWithComma(mMainHighMaxValue.toStringAsFixed(fixedLength), -1),
  ...
  void drawSignal(Canvas canvas) {
    for (var index in signalIndices) {
      var point = datas![index];
      Paint arrowPaint = Paint()
        ..color = point.isSignalSell == true ? Colors.green : Colors.red
      ...
      double y = getMainY(point.isSignalSell == true ? point.high : point.low);

      Path arrowPath = Path();
      if (point.isSignalSell == true) {
        arrowPath.moveTo(x, y);
        arrowPath.lineTo(x - arrowSize / 2, y - arrowHeight);
        arrowPath.lineTo(x + arrowSize / 2, y - arrowHeight);
        arrowPath.close();
      } else {
        arrowPath.moveTo(x, y);
      ...
      TextPainter tp = getTextPainter(
        '${point.signal}${formatWithComma(point.price.toString(), -1)}',
        chartColors.signalColor,
      );
      ...
      tp.paint(
        ...
        Offset(
          x - tp.width / 2,
          point.isSignalSell == true ? (y - arrowHeight - textOffset - tp.height) : (y + arrowHeight + textOffset),
        ),
      );
    }
  }
  ...
  void drawNowPrice(Canvas canvas) {
    ...
    TextPainter tp = getTextPainter(formatWithComma(value.toStringAsFixed(fixedLength), -1), this.chartColors.nowPriceTextColor);
